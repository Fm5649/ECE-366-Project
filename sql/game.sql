...
CREATE TABLE game(
  game_id bigint NOT NULL DEFAULT nextval('user_seq'),
  ...
  PRIMARY KEY (game_id),
  FOREIGN KEY (p1_id) REFERENCES user(user_id) ON DELETE CASCADE,
  FOREIGN KEY (p2_id) REFERENCES user(user_id) ON DELETE CASCADE,
  CHECK (p1_id <> p2_id),
